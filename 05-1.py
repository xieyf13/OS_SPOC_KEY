...
class scheduler:
    ...
    def move_to_ready(self, expected, pid=-1):
        #YOUR CODE
        if pid == -1:
            tmp_id = self.curr_proc
        else:
            tmp_id = pid
        if self.proc_info[tmp_id][PROC_STATE] == expected:
            self.proc_info[tmp_id][PROC_STATE] = STATE_READY
        return
    ...
    def next_proc(self, pid=-1):
        ...
        if pid == -1:
            tmp_id = (self.curr_proc + 1) % procNum
        else:
            tmp_id = (pid + 1) % procNum

        while self.proc_info[tmp_id][PROC_STATE] == STATE_DONE:
            tmp_id = (tmp_id + 1) % procNum
        self.curr_proc = tmp_id
