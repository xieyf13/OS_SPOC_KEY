...
parser.add_option("-p", "--policy", default="SJF", help="sched policy to use: SJF, FIFO, RR",
...
if options.solve == True:
    ...
    if options.policy == 'SJF':
        ...
        for job in joblist:
            print ("[ time   %d ] Run job %d for %.2f secs ( DONE at %.2f)"%(curtime, job[0], job[1], curtime + job[1]))
            curtime += job[1]
        print '\nFinal statistics:'
        t     = 0.0
        count = 0
