...
class fs:
    ...
    def deleteFile(self, tfile):
        ...
        file_name = tfile[len(tfile)-1]
        parent_inode = self.inodes[self.nameToInum[parent_name]]
        parent_inode.decRefCnt()
        parent_block = self.data[parent_inode.getAddr()]
        parent_block.delDirEntry(file_name)
    ...
    def createLink(self, target, newfile, parent):
    	...
    	parent_id = self.nameToInum[parent]
    	tinum = self.nameToInum[target]

    	if self.data[parent_id].dirEntryExists(newfile):
            print "createLink: %s already exists in %s" %newfile,parent
        else:
            self.data[parent_id].addDirEntry(newfile, tinum)

        self.inodes[parent_id].incRefCnt()
    ...
    def createFile(self, parent, newfile, ftype):
    	...
    	if ftype == 'd':
    		...
    		self.data[addr].addDirEntry("..", parent_id)
    	else:
    		addr = -1
    		self.inodes[inum].setAll(ftype, addr, 1)

    	self.nameToInum[newfile] = inum
    	self.data[parent_id].addDirEntry(newfile, inum)
    	self.inodes[parent_id].incRefCnt()
    	return inum
